	...
	(let ((guerreiro (make-instance 'guerreiro :nome "Jones" :hp 100 :forca 15 :magia 0))
		...
		(ltk:with-ltk ()
			...
			(let* ((lbl-titulo (make-instance 'ltk:label :text "=== CANDY BOX: LISP MUTANTE ===" :font "Courier 14 bold"))
				...
				;; Caixas de texto padrão (Entry) para exibir as funções mutantes na RAM
				...
				;; Função auxiliar interna para atualizar as caixas de texto com segurança
				...
				;; O GRANDE BOTÃO: Controla o passo a passo do jogo
				(btn-turno (make-instance 'ltk:button :text " >>> AVANÇAR TURNO (ATACAR) <<< " :font "Helvetica 11 bold"
											:command (lambda ()
														(if (<= (hp chefe) 0)
															(progn
				...
				;; Botões interceptores de ambiente
				(btn-normal (make-instance 'ltk:button :text "Mudar para Clima Normal"
											:command (lambda () 
														(mudar-clima-do-mundo 1)
														(setf (ltk:text lbl-clima) (format nil "Clima Atual: ~A" *clima-atual*))
														(funcall atualizar-interface-ast))))
				(btn-fogo (make-instance 'ltk:button :text "Ativar Clima Infernal (Fogo)"
											:command (lambda () 
														(mudar-clima-do-mundo 2)
														(setf (ltk:text lbl-clima) (format nil "Clima Atual: ~A" *clima-atual*))
														(funcall atualizar-interface-ast))))
				(btn-antimagia (make-instance 'ltk:button :text "Ativar Zona Antimagia"
												:command (lambda () 
				...
				;; Preenche os dados da AST na abertura da janela
				(funcall atualizar-interface-ast))))))